...
from fairseq.modules import LayerNorm, SMGKAttention
...
class SMGKTransformerEncoderLayerBase(nn.Module):
    ...
    def build_self_attention(self, embed_dim, cfg):
        return SMGKAttention(
            embed_dim,
            num_heads = cfg.encoder.attention_heads,
            head_dim = (embed_dim//cfg.encoder.attention_heads)/2,
...
class SMGKTransformerEncoderLayer(SMGKTransformerEncoderLayerBase):
    def __init__(self, args):
        super().__init__(SMGKTransformerConfig.from_namespace(args))
        self.args = args

    def build_self_attention(self, embed_dim, args):
        return super().build_self_attention(
            embed_dim, SMGKTransformerConfig.from_namespace(args)
        )


class SMGKTransformerDecoderLayerBase(nn.Module):
    """Decoder layer block.
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def build_self_attention(
        ...
        return SMGKAttention(
            ...
            head_dim = (embed_dim//cfg.decoder.attention_heads)/2,
            dropout=cfg.attention_dropout,
            add_bias_kv=add_bias_kv,
            add_zero_attn=add_zero_attn,
...
# backward compatible with the legacy argparse format
class SMGKTransformerDecoderLayer(SMGKTransformerDecoderLayerBase):
    def __init__(
        self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False
    ):
        super().__init__(
            SMGKTransformerConfig.from_namespace(args),
