...
from utils.cuda_fast_weight_layer import CudaFastWeightFishTransformerLayer
...
class HDPAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, n_global_head, dropatt=0, 
        ...
        self.sigma_noise = nn.Parameter(0.1 * torch.ones(n_global_head), requires_grad= True)
    ...
    def forward(self, h, attn_mask=None, mems=None,
        ...
        # add noise
        # attn_score = attn_score + (self.sigma_noise**2) * torch.randn_like(attn_score)
        attn_score = attn_score + ((self.sigma_noise**2)[None, None, None, :]) * torch.randn_like(attn_score)
        ###########
...
# Mixture attention.
class MixtureAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, n_global_head, dropatt=0, 
                 pre_lnorm=False):
        super(MixtureAttn, self).__init__()
        ...
        self.k_net = nn.Linear(d_model, self.n_global_head * d_head, bias=False)
        self.v_net = nn.Linear(d_model, self.n_head  * d_head, bias=False)
        self.hdp_net = nn.Linear(self.n_global_head, 1, bias=False)
#         self.hdp_net = nn.Sequential(
#             nn.Linear(self.n_global_head, self.n_head, bias=True),
#             nn.ReLU(),
#             nn.Linear(self.n_head, self.n_head, bias=True)
#         )
    ...
    def forward(self, h, attn_mask=None, mems=None,
        ...
        # generate local heads
        attn_list = []
        for i in range(self.n_head):
            attn_list.append(self.hdp_net(attn_score + (self.sigma_noise**2) * torch.randn_like(attn_score)))
...
class DecoderLayer(nn.Module):
    def __init__(self, n_head, d_model, d_head, d_inner, dropout, attn_type,
        ...
        if attn_type == 2:
            ...
            attn_func = MixtureAttn
        elif attn_type == 234:
            attn_func = CudaFastWeightFishTransformerLayer
        else:
...
class MemTransformerLM(nn.Module):
    def __init__(self,
        ...
        elif attn_type in [205, 206]:  # absolute embeddings
            for i in range(n_layer):
                self.layers.append(
                    DecoderLayer(
        ...
        elif attn_type in [10, 14, 24, 34, 44, 234]:  # fast weights
        ...
        if self.attn_type == 0:  # default attention
            ...
            self.r_bias = nn.Parameter(torch.Tensor(
                    self.n_layer, self.max_klen, self.n_head))
                                ...
                                24, 25, 26,
                                34, 35,
                                44, 45, 46, 200, 201, 202, 203, 204, 205, 206, 234]:
    ...
    def _forward(self, dec_inp, mems=None, carry_over_fast_weight=False):
        ...
        elif self.attn_type in [2, 4, 5, 6, 7, 10, 14, 16, 200, 201, 202, 203, 204, 205, 206]:  # absolute
            if self.no_pos:
                core_out = self.drop(word_emb)
            else:
        ...
        elif self.attn_type in [24, 25, 26, 34, 35, 44, 45, 46, 234]:  # absolute
            if self.no_pos:
                core_out = self.drop(word_emb)
            else:
                pos_seq = torch.arange(klen-1, -1, -1.0,
